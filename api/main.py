"""Nexus911 — FastAPI Application."""
import json
import logging
from contextlib import asynccontextmanager
from fastapi import FastAPI, WebSocket, WebSocketDisconnect
from fastapi.middleware.cors import CORSMiddleware
from fastapi.responses import HTMLResponse
from fastapi.staticfiles import StaticFiles
from pathlib import Path
from core.config import settings
from core.incident_graph import incident_manager
from api.routes import router
from api.voice_websocket import handle_voice_websocket
from api.simulation_routes import router as simulation_router
from verification.middleware import router as verify_router
from services.voice_session_manager import voice_manager


logging.basicConfig(level=logging.INFO, format="%(asctime)s | %(name)s | %(levelname)s | %(message)s")
logger = logging.getLogger("nexus911")

FRONTEND_DIR = Path(__file__).parent.parent / "frontend"
FRONTEND_DIST = FRONTEND_DIR / "dist"


@asynccontextmanager
async def lifespan(app: FastAPI):
    logger.info("Starting Nexus911 v1.0.0")
    yield
    logger.info("Shutting down Nexus911")


app = FastAPI(
    title="Nexus911",
    description="Multi-Agent Emergency Coordination System powered by Gemini Live API",
    version="1.0.0",
    lifespan=lifespan,
)

app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
    allow_credentials=True,
    allow_methods=["*"],
    allow_headers=["*"],
)

app.include_router(router, prefix="/api")
app.include_router(verify_router, prefix="/api")
app.include_router(simulation_router, prefix="/api")


@app.websocket("/ws/dashboard")
async def dashboard_websocket(websocket: WebSocket):
    await websocket.accept()
    incident_manager.add_listener(websocket)
    logger.info("Dashboard client connected")
    try:
        while True:
            data = await websocket.receive_text()
            msg = json.loads(data)
            if msg.get("type") == "get_all_incidents":
                incidents = {iid: inc.to_dict() for iid, inc in incident_manager.incidents.items()}
                await websocket.send_text(
                    json.dumps({"type": "all_incidents", "data": incidents}, default=str)
                )
    except WebSocketDisconnect:
        incident_manager.remove_listener(websocket)
        logger.info("Dashboard client disconnected")


@app.get("/health")
async def health():
    return {
        "status": "healthy",
        "version": "1.0.0",
        "active_incidents": len(incident_manager.get_active_incidents()),
        "active_voice_sessions": voice_manager.active_count,
    }


@app.get("/api/voice/sessions")
async def get_voice_sessions():
    """Get all active voice sessions for the dashboard."""
    return {"sessions": voice_manager.get_active_sessions()}


@app.websocket("/ws/voice")
async def voice_websocket(websocket: WebSocket):
    await handle_voice_websocket(websocket)


# ── Simulation event listeners ──────────────────────────────
_sim_listeners: list[WebSocket] = []


@app.websocket("/ws/simulation")
async def simulation_websocket(websocket: WebSocket):
    """Real-time simulation events: transcripts, tool calls, audio, status."""
    await websocket.accept()
    _sim_listeners.append(websocket)
    logger.info("Simulation listener connected")
    try:
        while True:
            await websocket.receive_text()  # Keep-alive
    except WebSocketDisconnect:
        pass
    finally:
        if websocket in _sim_listeners:
            _sim_listeners.remove(websocket)
        logger.info("Simulation listener disconnected")


async def broadcast_sim_event(event: dict):
    """Broadcast a simulation event to all connected listeners."""
    dead = []
    msg = json.dumps(event, default=str)
    for ws in _sim_listeners:
        try:
            await ws.send_text(msg)
        except Exception:
            dead.append(ws)
    for ws in dead:
        if ws in _sim_listeners:
            _sim_listeners.remove(ws)


# Serve the built React frontend (production) or dev index.html (development)
if FRONTEND_DIST.exists():
    # Production: serve the Vite build output
    @app.get("/", response_class=HTMLResponse)
    async def root():
        index_path = FRONTEND_DIST / "index.html"
        return HTMLResponse(content=index_path.read_text(encoding="utf-8"))

    app.mount("/assets", StaticFiles(directory=str(FRONTEND_DIST / "assets")), name="static-assets")
else:
    @app.get("/", response_class=HTMLResponse)
    async def root():
        return HTMLResponse(
            content=(
                "<h1>Nexus911</h1>"
                "<p>Frontend not built. Run <code>cd frontend && npm install && npm run build</code> "
                "or use Vite dev server at <code>http://localhost:5173</code>.</p>"
                "<p><a href='/docs'>API Documentation</a></p>"
            )
        )
